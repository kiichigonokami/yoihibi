<!-- 商品詳細自由項目１ -->
<style>
  .camp_title_area img,
  .camp_middle_img img,
  .camp_koe_area img {width: 100%;}
  .camp_title_bottom span { display: block;}
  .camp_contents { display: block; overflow: hidden; margin: 0 0 0 0; padding: 0 0 0 0; clear: both;}
  .camp_contents_l { display: block; overflow: hidden; float: left; margin: 0 0 0 0; padding: 0 0 0 0; width: 50%;}
  .camp_contents_r { display: block; overflow: hidden; float: right; margin: 0 0 0 0; padding: 0 0 0 0; width: 50%;}
  .camp_contents_box { display: block; overflow: hidden; margin: 0 0 0 0; padding: 0 0 0 0; clear: both;}
  .camp_head { display: block; overflow: hidden; padding: 0 0 0 0; margin: 0 0 24px 0;}
  .camp_title_bottom {font-size: 32px; color: #333; line-height: 1.2; padding: 16px 24px 0; text-align: justify;}
  .camp_head blockquote {line-height: 1.6; padding: 0 16px 0; text-align: justify; clear: both:}
  .camp_middle_img { margin: 0 24px 0 }
  .camp_koe_area { margin: 0 24px 0;}
  @media screen and (max-width: 767px) {
    .camp_contents { margin: 0 0 24px 0; padding: 0 0 0 0;}
    .camp_contents_l { float: inherit; margin: 0 0 0 0; padding: 0 0 0 0; width: 100%;}
    .camp_contents_r { float: inherit; margin: 0 0 0 0; padding: 0 0 0 0; width: 100%;}
    .camp_contents_box { margin: 0 0 0px 0;}
    .camp_head { display: block; overflow: hidden; padding: 0 0 0 0; margin: 0 0 16px 0;}
    .camp_title_bottom {font-size: 24px; padding: 8px 16px 0;}
    .camp_title_bottom span small { top: 0!important;}
    .camp_head blockquote {padding: 16px 16px 8px;}
    .camp_middle_img { margin: 0 0 0 0;}
    .camp_koe_area { margin: 0 0 0;}
    .camp_title_bottom span { display: inline;}
  }
</style>

<div class="item_detail_section_wrap2">
  <h2 class="ttl_flower">すこやかに休んで、思考力も回復！ </h2>
  <div class="l_columns_w item_detail_sub_box">
    <div class="item_detail_section">

      <div class="camp_contents">
        <div class="camp_contents_l">
          <div class="camp_contents_box">
            <div class="camp_head">
              <blockquote>
                <b>緊張している心もからだも、ほっとひと息</b><br>
                睡眠リズムが乱れたり、寝不足が続くとからだが重くなるだけでなく、思考力まで鈍って低下してしまいます。そんな時は『リラックス・エッセンス』で、クリアな思考を取り戻しましょう。オーガニック栽培された６種のハーブが互いの力を引き出しながら、穏やかなお休みをサポートし、自然に目覚める喜びを味わえます。
              </blockquote>
            </div>
            <div class="camp_middle">
              <p class="camp_middle_img"><img src="https://aladdinec.jp/client_info/FLORAHOUSE01/view/userweb/images/feature/1901CAMP/etc4.png"></p>
            </div>
            <br>
            <div class="camp_middle">
              <p class="camp_middle_img"><img src="https://aladdinec.jp/client_info/FLORAHOUSE01/view/userweb/images/feature/1901CAMP/etc5.png"></p>
            </div>
          </div>
        </div>
        <div class="camp_contents_r">
          <div class="camp_contents_box">
            <div class="camp_middle">
              <p class="camp_middle_img"><img src="https://aladdinec.jp/client_info/FLORAHOUSE01/view/userweb/images/feature/1901CAMP/etcl1_sp.png"></p>
            </div>
          </div>
        </div>
      </div>

    </div>
  </div>
</div>